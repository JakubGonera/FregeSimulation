...
\<comment> \<open>Gap-region bound: rebal_cn = +2 raises rebal_L_step's threshold to
    4*rebal_kk+4, while spira_threshold (= 2*rebal_kk+2 for MaxArity \<ge> 2) cannot
    be bumped to match (Translation.thy uses spira_threshold's exact value).  So
    formulas P with spira_threshold \<le> |P| < 4*rebal_kk+4 sit in a gap where the
    recurrence has no contraction --- but they are also non-recursive in the
    sense that |P| is bounded by a constant.  rebal_gapc captures a uniform proof
    complexity for the bounded family, proved below by induction on rebal_measure
    using a closed-form recursive bound rebal_gap_bnd.\<close>

\<comment> \<open>Sbound: uniform upper bound on len_formula (spira_trans P) and
    depth_formula (spira_trans P) for formulas P with |P| < 4 * rebal_kk + 4.\<close>
definition rebal_gap_Sbound :: nat where
  "rebal_gap_Sbound = poly rebal_tb (4 * rebal_kk + 3)"

lemma rebal_gap_spira_len_bound:
  assumes wfP: "formula_well_formed (alphabet F) P"
      and ltP: "len_formula P < 4 * rebal_kk + 4"
  shows "len_formula (spira_trans P) \<le> rebal_gap_Sbound"
proof -
  have lle: "len_formula P \<le> 4 * rebal_kk + 3" using ltP by simp
  have "len_formula (spira_trans P) \<le> poly rebal_tb (len_formula P)"
    by (rule rebal_tb_spec[OF wfP])
  also have "\<dots> \<le> poly rebal_tb (4 * rebal_kk + 3)"
    using lle by (rule poly_nat_mono)
  finally show ?thesis unfolding rebal_gap_Sbound_def .
qed

lemma rebal_gap_spira_dep_bound:
  assumes wfP: "formula_well_formed (alphabet F) P"
      and ltP: "len_formula P < 4 * rebal_kk + 4"
  shows "depth_formula (spira_trans P) \<le> rebal_gap_Sbound"
proof -
  have "depth_formula (spira_trans P) \<le> len_formula (spira_trans P)"
    by (rule depth_formula_le_len)
  also have "\<dots> \<le> rebal_gap_Sbound"
    by (rule rebal_gap_spira_len_bound[OF wfP ltP])
  finally show ?thesis .
qed

\<comment> \<open>K_base: dominates every non-recursive complexity contribution
    (eq, pos=[], below-threshold).\<close>
definition rebal_gap_K_base :: nat where
  "rebal_gap_K_base =
     refl_lines + refl_step_len * rebal_gap_Sbound + refl_step_depth + rebal_gap_Sbound
     + pos_empty_lines + pos_empty_step_len * rebal_gap_Sbound
     + pos_empty_step_depth + 1 + rebal_gap_Sbound
     + rebal_shc + 1"

\<comment> \<open>K_step: dominates every glue contribution per recursive step.  Lines glue is
    case_*_glue_lines.  Size glue is the polynomial leaf sum times glue_coeff
    (8 or 10 leaves, each \<le> Sbound).  Depth glue is dep_coeff * (sum + 1).\<close>
definition rebal_gap_K_step :: nat where
  "rebal_gap_K_step =
     case_one_glue_lines + case_two_glue_lines + case_three_glue_lines
     + rebal_glue_coeff * (8 * rebal_gap_Sbound + 1)
     + rebal_glue_coeff3 * (10 * rebal_gap_Sbound + 1)
     + rebal_dep_coeff * (8 * rebal_gap_Sbound + 1)
     + rebal_dep_coeff3 * (10 * rebal_gap_Sbound + 1)
     + 1"

\<comment> \<open>Closed-form bound: (K_base + K_step) * 5^n satisfies
    4 * bnd n + K_step \<le> bnd (Suc n).\<close>
...
lemma rebal_gap_bnd_mono:
  ...
  unfolding rebal_gap_bnd_def
  using assms by (intro mult_le_mono2 power_increasing) auto
...
lemma rebal_gap_bnd_step:
  shows "4 * rebal_gap_bnd n + rebal_gap_K_step \<le> rebal_gap_bnd (Suc n)"
proof -
  let ?K = "rebal_gap_K_base + rebal_gap_K_step"
  have p5: "(1::nat) \<le> 5 ^ n" by simp
  have step_le: "rebal_gap_K_step \<le> ?K * 5 ^ n"
  proof -
    have "rebal_gap_K_step \<le> ?K" by simp
    also have "\<dots> = ?K * 1" by simp
    also have "\<dots> \<le> ?K * 5 ^ n" using p5 by (intro mult_le_mono2)
    finally show ?thesis .
  qed
  have "4 * rebal_gap_bnd n + rebal_gap_K_step
      = 4 * (?K * 5 ^ n) + rebal_gap_K_step"
    unfolding rebal_gap_bnd_def by simp
  also have "\<dots> \<le> 4 * (?K * 5 ^ n) + ?K * 5 ^ n"
    using step_le by simp
  also have "\<dots> = ?K * (5 * 5 ^ n)" by simp
  also have "\<dots> = rebal_gap_bnd (Suc n)" unfolding rebal_gap_bnd_def by simp
  finally show ?thesis .
qed

\<comment> \<open>The auxiliary lemma: by induction on rebal_measure, every (P, pos) with
    |P| < 4*rebal_kk+4 has a balanced-iff proof of complexity bounded by
    rebal_gap_bnd of the measure.  Both base cases (eq, pos=[]) and the
    below-threshold case are dominated by K_base; recursive trichotomy cases
    feed three or four IHs into rebal_gap_bnd_step.\<close>
lemma rebal_gap_aux:
  fixes P :: "'c formula" and pos :: "nat list"
  assumes "formula_well_formed (alphabet F) P"
      and "valid_position P pos"
...
proof (induction "rebal_measure P pos" arbitrary: P pos rule: less_induct)
  case less
  have wfP: "formula_well_formed (alphabet F) P" using less.prems(1) .
  have vpP: "valid_position P pos" using less.prems(2) .
  have ltP: "len_formula P < 4 * rebal_kk + 4" using less.prems(3) .

  have Sb: "len_formula (spira_trans P) \<le> rebal_gap_Sbound"
    by (rule rebal_gap_spira_len_bound[OF wfP ltP])
  have Db: "depth_formula (spira_trans P) \<le> rebal_gap_Sbound"
    by (rule rebal_gap_spira_dep_bound[OF wfP ltP])

  let ?meas = "rebal_measure P pos"
  let ?bnd = "rebal_gap_bnd ?meas"

  \<comment> \<open>Base helpers: refl-case and pos=[] case both stay within K_base.\<close>
  have refl_case: "rebalancing P pos = spira_trans P \<Longrightarrow>
        (\<exists> lines sz dep.
           provable_balanced_iff (spira_trans P) (rebalancing P pos) lines sz dep
         \<and> lines \<le> ?bnd \<and> sz \<le> ?bnd \<and> dep \<le> ?bnd)"
  proof -
    ...
    have lb: "refl_lines \<le> ?bnd"
    proof -
      have "refl_lines \<le> rebal_gap_K_base"
        unfolding rebal_gap_K_base_def by simp
      thus ?thesis using rebal_gap_K_base_le_bnd by (rule order_trans)
    qed
    have sb: "refl_step_len * len_formula (spira_trans P) \<le> ?bnd"
    proof -
      have "refl_step_len * len_formula (spira_trans P)
            \<le> refl_step_len * rebal_gap_Sbound" using Sb by simp
      also have "\<dots> \<le> rebal_gap_K_base"
        unfolding rebal_gap_K_base_def by simp
      also have "\<dots> \<le> ?bnd" by (rule rebal_gap_K_base_le_bnd)
      finally show ?thesis .
    qed
    have db: "refl_step_depth + depth_formula (spira_trans P) \<le> ?bnd"
    proof -
      have "refl_step_depth + depth_formula (spira_trans P)
            \<le> refl_step_depth + rebal_gap_Sbound" using Db by simp
      also have "\<dots> \<le> rebal_gap_K_base"
        unfolding rebal_gap_K_base_def by simp
      also have "\<dots> \<le> ?bnd" by (rule rebal_gap_K_base_le_bnd)
      finally show ?thesis .
  ...
  have pos_empty_case: "pos = [] \<Longrightarrow>
        (\<exists> lines sz dep.
           provable_balanced_iff (spira_trans P) (rebalancing P pos) lines sz dep
         \<and> lines \<le> ?bnd \<and> sz \<le> ?bnd \<and> dep \<le> ?bnd)"
  proof -
    ...
    have pbi: "provable_balanced_iff (spira_trans P) (rebalancing P pos)
                 ...
                 (pos_empty_step_len * len_formula (spira_trans P))
                 (pos_empty_step_depth + 1 + depth_formula (spira_trans P))"
      using case_pos_empty_construction[of P] pe by simp
    have lb: "pos_empty_lines \<le> ?bnd"
    proof -
      have "pos_empty_lines \<le> rebal_gap_K_base"
        unfolding rebal_gap_K_base_def by simp
      thus ?thesis using rebal_gap_K_base_le_bnd by (rule order_trans)
    qed
    have sb: "pos_empty_step_len * len_formula (spira_trans P) \<le> ?bnd"
    proof -
      have "pos_empty_step_len * len_formula (spira_trans P)
            \<le> pos_empty_step_len * rebal_gap_Sbound" using Sb by simp
      also have "\<dots> \<le> rebal_gap_K_base"
        unfolding rebal_gap_K_base_def by simp
      also have "\<dots> \<le> ?bnd" by (rule rebal_gap_K_base_le_bnd)
      finally show ?thesis .
    qed
    have db: "pos_empty_step_depth + 1 + depth_formula (spira_trans P) \<le> ?bnd"
    proof -
      have "pos_empty_step_depth + 1 + depth_formula (spira_trans P)
            \<le> pos_empty_step_depth + 1 + rebal_gap_Sbound" using Db by simp
      also have "\<dots> \<le> rebal_gap_K_base"
        unfolding rebal_gap_K_base_def by simp
      also have "\<dots> \<le> ?bnd" by (rule rebal_gap_K_base_le_bnd)
      finally show ?thesis .
  ...
  proof (cases "pos = []")
    case True
    show ?thesis using pos_empty_case[OF True] .
  next
    case pos_ne: False
    show ?thesis
    proof (cases "len_formula P < spira_threshold")
      case True
      \<comment> \<open>Below threshold: rebal_shc_spec, bounded by rebal_shc \<le> K_base.\<close>
      obtain lines sz dep where
          sc: "provable_balanced_iff (spira_trans P) (rebalancing P pos)
                 lines sz dep"
        and scl: "lines \<le> rebal_shc" and scs: "sz \<le> rebal_shc"
        and scd: "dep \<le> rebal_shc"
        using rebal_shc_spec[OF wfP vpP True] by blast
      have shc_le: "rebal_shc \<le> ?bnd"
      proof -
        have "rebal_shc \<le> rebal_gap_K_base"
          unfolding rebal_gap_K_base_def by simp
        thus ?thesis using rebal_gap_K_base_le_bnd by (rule order_trans)
      qed
      have lb: "lines \<le> ?bnd" using scl shc_le by linarith
      have sb: "sz \<le> ?bnd" using scs shc_le by linarith
      have db: "dep \<le> ?bnd" using scd shc_le by linarith
      show ?thesis using sc lb sb db by blast
    next
      case bigST: False
      have geST: "len_formula P \<ge> spira_threshold" using bigST by simp
      \<comment> \<open>In the gap proper: trichotomy on (spiras_sel_position P, pos).\<close>
      ...
      proof cases
        ...
        thus ?thesis using refl_case by simp
      next
        case rbelow
        ...
        \<comment> \<open>Sub-problem length bounds (all \<le> |P| < 4*rebal_kk+4).\<close>
        have lQ_le_P: "len_formula (subterm_at P (spiras_sel_position P))
                       \<le> len_formula P"
          using subterm_at_len_le[OF conjunct1[OF split]] .
        have lT_le_P: "len_formula (fix_at pos True P) \<le> len_formula P"
          by (rule fix_at_len_le)
        have lF_le_P: "len_formula (fix_at pos False P) \<le> len_formula P"
          by (rule fix_at_len_le)
        have ltQ: "len_formula (subterm_at P (spiras_sel_position P))
                   < 4 * rebal_kk + 4"
          using lQ_le_P ltP by linarith
        have ltT: "len_formula (fix_at pos True P) < 4 * rebal_kk + 4"
          using lT_le_P ltP by linarith
        have ltF: "len_formula (fix_at pos False P) < 4 * rebal_kk + 4"
          using lF_le_P ltP by linarith
        \<comment> \<open>Three IHs, bounded by rebal_gap_bnd of strictly smaller measures.\<close>
        obtain lQ szQ depQ where IH_Q:
            "provable_balanced_iff
               (spira_trans (subterm_at P (spiras_sel_position P)))
               (rebalancing (subterm_at P (spiras_sel_position P)) s) lQ szQ depQ"
          and IH_Q_l: "lQ \<le> rebal_gap_bnd
                        (rebal_measure (subterm_at P (spiras_sel_position P)) s)"
          and IH_Q_s: "szQ \<le> rebal_gap_bnd
                        (rebal_measure (subterm_at P (spiras_sel_position P)) s)"
          and IH_Q_d: "depQ \<le> rebal_gap_bnd
                        (rebal_measure (subterm_at P (spiras_sel_position P)) s)"
          using less.hyps[OF conjunct1[OF meas] wfQ conjunct2[OF split] ltQ]
          by blast
        obtain lT szT depT where IH_T:
            "provable_balanced_iff (spira_trans (fix_at pos True P))
               (rebalancing (fix_at pos True P) (spiras_sel_position P))
               lT szT depT"
          and IH_T_l: "lT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos True P) (spiras_sel_position P))"
          and IH_T_s: "szT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos True P) (spiras_sel_position P))"
          and IH_T_d: "depT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos True P) (spiras_sel_position P))"
          using less.hyps[OF conjunct1[OF conjunct2[OF meas]]
                           fix_at_wf[OF wfP] vpT ltT] by blast
        obtain lF szF depF where IH_F:
            "provable_balanced_iff (spira_trans (fix_at pos False P))
               (rebalancing (fix_at pos False P) (spiras_sel_position P))
               lF szF depF"
          and IH_F_l: "lF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos False P) (spiras_sel_position P))"
          and IH_F_s: "szF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos False P) (spiras_sel_position P))"
          and IH_F_d: "depF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos False P) (spiras_sel_position P))"
          using less.hyps[OF conjunct2[OF conjunct2[OF meas]]
                           fix_at_wf[OF wfP] vpF ltF] by blast
        let ?Q = "subterm_at P (spiras_sel_position P)"
        obtain sz dep where pbi:
            "provable_balanced_iff (spira_trans P) (rebalancing P pos)
               (lQ + lT + lF + case_one_glue_lines) sz dep"
          and conS: "sz \<le> szQ + szT + szF + rebal_glue_coeff
              * (len_formula (spira_trans (fix_at (spiras_sel_position P) True P))
                 + len_formula (spira_trans (fix_at (spiras_sel_position P) False P))
                 ...
                 + len_formula (spira_trans (fix_at pos True P))
                 + len_formula (spira_trans (fix_at pos False P)) + 1)"
          and conD: "dep \<le> max depQ (max depT (max depF (rebal_dep_coeff
              * (depth_formula (spira_trans (fix_at (spiras_sel_position P) True P))
                 + depth_formula (spira_trans (fix_at (spiras_sel_position P) False P))
                 ...
                 + depth_formula (spira_trans (fix_at pos True P))
                 + depth_formula (spira_trans (fix_at pos False P)) + 1))))"
          using case_one_construction[OF wfP geST pos_eq vpP IH_Q IH_T IH_F]
          by blast

        \<comment> \<open>Measure is Suc of something; let m1 = its predecessor.\<close>
        have meas_pos: "0 < ?meas"
        proof -
          have "0 < len_formula P * (len_formula P + 1)"
            using len_formula_positive[of P] by simp
          thus ?thesis unfolding rebal_measure_def by simp
        qed
        then obtain m1 where m1_eq: "?meas = Suc m1" by (cases ?meas) auto
        have mQ_le: "rebal_measure (subterm_at P (spiras_sel_position P)) s \<le> m1"
          using conjunct1[OF meas] m1_eq by simp
        have mT_le: "rebal_measure (fix_at pos True P) (spiras_sel_position P) \<le> m1"
          using conjunct1[OF conjunct2[OF meas]] m1_eq by simp
        have mF_le: "rebal_measure (fix_at pos False P) (spiras_sel_position P) \<le> m1"
          using conjunct2[OF conjunct2[OF meas]] m1_eq by simp

        \<comment> \<open>Each IH bounded by rebal_gap_bnd m1.\<close>
        have lQ_b: "lQ \<le> rebal_gap_bnd m1"
          using IH_Q_l rebal_gap_bnd_mono[OF mQ_le] by (rule order_trans)
        have lT_b: "lT \<le> rebal_gap_bnd m1"
          using IH_T_l rebal_gap_bnd_mono[OF mT_le] by (rule order_trans)
        have lF_b: "lF \<le> rebal_gap_bnd m1"
          using IH_F_l rebal_gap_bnd_mono[OF mF_le] by (rule order_trans)
        have sQ_b: "szQ \<le> rebal_gap_bnd m1"
          using IH_Q_s rebal_gap_bnd_mono[OF mQ_le] by (rule order_trans)
        have sT_b: "szT \<le> rebal_gap_bnd m1"
          using IH_T_s rebal_gap_bnd_mono[OF mT_le] by (rule order_trans)
        have sF_b: "szF \<le> rebal_gap_bnd m1"
          using IH_F_s rebal_gap_bnd_mono[OF mF_le] by (rule order_trans)
        have dQ_b: "depQ \<le> rebal_gap_bnd m1"
          using IH_Q_d rebal_gap_bnd_mono[OF mQ_le] by (rule order_trans)
        have dT_b: "depT \<le> rebal_gap_bnd m1"
          using IH_T_d rebal_gap_bnd_mono[OF mT_le] by (rule order_trans)
        have dF_b: "depF \<le> rebal_gap_bnd m1"
          using IH_F_d rebal_gap_bnd_mono[OF mF_le] by (rule order_trans)

        \<comment> \<open>Leaf bounds: each spira leaf \<le> Sbound.\<close>
        have leafSb: "\<And>L. formula_well_formed (alphabet F) L \<Longrightarrow>
                          len_formula L \<le> len_formula P \<Longrightarrow>
                          len_formula (spira_trans L) \<le> rebal_gap_Sbound"
        proof -
          fix L assume wfL: "formula_well_formed (alphabet F) L"
                       and lLP: "len_formula L \<le> len_formula P"
          have "len_formula L < 4 * rebal_kk + 4" using lLP ltP by linarith
          thus "len_formula (spira_trans L) \<le> rebal_gap_Sbound"
            by (rule rebal_gap_spira_len_bound[OF wfL])
        qed
        have leafDb: "\<And>L. formula_well_formed (alphabet F) L \<Longrightarrow>
                          len_formula L \<le> len_formula P \<Longrightarrow>
                          depth_formula (spira_trans L) \<le> rebal_gap_Sbound"
        proof -
          fix L assume wfL: "formula_well_formed (alphabet F) L"
                       and lLP: "len_formula L \<le> len_formula P"
          have "len_formula L < 4 * rebal_kk + 4" using lLP ltP by linarith
          thus "depth_formula (spira_trans L) \<le> rebal_gap_Sbound"
            by (rule rebal_gap_spira_dep_bound[OF wfL])
        qed
        have wf_Q: "formula_well_formed (alphabet F) ?Q" by (rule wfQ)
        have wf_T: "formula_well_formed (alphabet F) (fix_at pos True P)"
          by (rule fix_at_wf[OF wfP])
        have wf_F: "formula_well_formed (alphabet F) (fix_at pos False P)"
          by (rule fix_at_wf[OF wfP])
        have wf_Xq_T: "formula_well_formed (alphabet F)
                        (fix_at (spiras_sel_position P) True P)"
          by (rule fix_at_wf[OF wfP])
        have wf_Xq_F: "formula_well_formed (alphabet F)
                        (fix_at (spiras_sel_position P) False P)"
          by (rule fix_at_wf[OF wfP])
        have wf_sQT: "formula_well_formed (alphabet F) (fix_at s True ?Q)"
          by (rule fix_at_wf[OF wfQ])
        have wf_sQF: "formula_well_formed (alphabet F) (fix_at s False ?Q)"
          by (rule fix_at_wf[OF wfQ])
        have wf_R: "formula_well_formed (alphabet F) (subterm_at P pos)"
          by (rule subterm_at_wf[OF wfP vpP])
        have lXqT: "len_formula (fix_at (spiras_sel_position P) True P)
                    \<le> len_formula P" by (rule fix_at_len_le)
        have lXqF: "len_formula (fix_at (spiras_sel_position P) False P)
                    \<le> len_formula P" by (rule fix_at_len_le)
        have lsQT: "len_formula (fix_at s True ?Q) \<le> len_formula P"
          using fix_at_len_le lQ_le_P by (rule order_trans)
        have lsQF: "len_formula (fix_at s False ?Q) \<le> len_formula P"
          using fix_at_len_le lQ_le_P by (rule order_trans)
        have lR_le: "len_formula (subterm_at P pos) \<le> len_formula P"
          by (rule subterm_at_len_le[OF vpP])

        \<comment> \<open>Eight-leaf sum bound for size glue.\<close>
        let ?llsum =
          "len_formula (spira_trans (fix_at (spiras_sel_position P) True P))
         + len_formula (spira_trans (fix_at (spiras_sel_position P) False P))
         + len_formula (spira_trans (fix_at s True ?Q))
         ...
         + len_formula (spira_trans (fix_at pos True P))
         + len_formula (spira_trans (fix_at pos False P))"
        have llsum_le: "?llsum \<le> 8 * rebal_gap_Sbound"
          using leafSb[OF wf_Xq_T lXqT] leafSb[OF wf_Xq_F lXqF]
                leafSb[OF wf_sQT lsQT] leafSb[OF wf_sQF lsQF]
                leafSb[OF wf_R lR_le] leafSb[OF wf_Q lQ_le_P]
                leafSb[OF wf_T lT_le_P] leafSb[OF wf_F lF_le_P]
          by linarith
        let ?dlsum =
          "depth_formula (spira_trans (fix_at (spiras_sel_position P) True P))
         + depth_formula (spira_trans (fix_at (spiras_sel_position P) False P))
         ...
         + depth_formula (spira_trans (fix_at pos True P))
         + depth_formula (spira_trans (fix_at pos False P))"
        have dlsum_le: "?dlsum \<le> 8 * rebal_gap_Sbound"
          using leafDb[OF wf_Xq_T lXqT] leafDb[OF wf_Xq_F lXqF]
                leafDb[OF wf_sQT lsQT] leafDb[OF wf_sQF lsQF]
                leafDb[OF wf_R lR_le] leafDb[OF wf_Q lQ_le_P]
                leafDb[OF wf_T lT_le_P] leafDb[OF wf_F lF_le_P]
          by linarith

        \<comment> \<open>Lines bound.\<close>
        have lines_le: "lQ + lT + lF + case_one_glue_lines \<le> ?bnd"
        proof -
          have "lQ + lT + lF + case_one_glue_lines
                \<le> 3 * rebal_gap_bnd m1 + case_one_glue_lines"
            using lQ_b lT_b lF_b by linarith
          also have "\<dots> \<le> 4 * rebal_gap_bnd m1 + rebal_gap_K_step"
          proof -
            have "case_one_glue_lines \<le> rebal_gap_K_step"
              unfolding rebal_gap_K_step_def by simp
            thus ?thesis by simp
          qed
          also have "\<dots> \<le> rebal_gap_bnd (Suc m1)" by (rule rebal_gap_bnd_step)
          also have "\<dots> = ?bnd" using m1_eq by simp
          finally show ?thesis .
        qed

        \<comment> \<open>Size bound.\<close>
        have sz_le: "sz \<le> ?bnd"
        proof -
          have "sz \<le> szQ + szT + szF + rebal_glue_coeff * (?llsum + 1)"
            by (rule conS)
          also have "\<dots> \<le> 3 * rebal_gap_bnd m1
                        + rebal_glue_coeff * (8 * rebal_gap_Sbound + 1)"
          proof -
            have c1: "szQ + szT + szF \<le> 3 * rebal_gap_bnd m1"
              using sQ_b sT_b sF_b by linarith
            have c2: "?llsum + 1 \<le> 8 * rebal_gap_Sbound + 1"
              using llsum_le by linarith
            have c3: "rebal_glue_coeff * (?llsum + 1)
                      \<le> rebal_glue_coeff * (8 * rebal_gap_Sbound + 1)"
              using c2 by simp
            from c1 c3 show ?thesis by linarith
          qed
          also have "\<dots> \<le> 4 * rebal_gap_bnd m1 + rebal_gap_K_step"
          proof -
            have "rebal_glue_coeff * (8 * rebal_gap_Sbound + 1)
                  \<le> rebal_gap_K_step"
              unfolding rebal_gap_K_step_def by simp
            thus ?thesis by simp
          qed
        ...
        have dep_le: "dep \<le> ?bnd"
        proof -
          have "dep \<le> max depQ (max depT (max depF
                       (rebal_dep_coeff * (?dlsum + 1))))" by (rule conD)
          also have "\<dots> \<le> rebal_gap_bnd m1
                        + rebal_dep_coeff * (8 * rebal_gap_Sbound + 1)"
          proof -
            have a1: "depQ \<le> rebal_gap_bnd m1" by (rule dQ_b)
            have a2: "depT \<le> rebal_gap_bnd m1" by (rule dT_b)
            have a3: "depF \<le> rebal_gap_bnd m1" by (rule dF_b)
            have a4: "rebal_dep_coeff * (?dlsum + 1)
                      \<le> rebal_dep_coeff * (8 * rebal_gap_Sbound + 1)"
              using dlsum_le by simp
            from a1 a2 a3 a4 show ?thesis by linarith
          qed
          also have "\<dots> \<le> 4 * rebal_gap_bnd m1 + rebal_gap_K_step"
          proof -
            have "rebal_dep_coeff * (8 * rebal_gap_Sbound + 1)
                  \<le> rebal_gap_K_step"
              unfolding rebal_gap_K_step_def by simp
            thus ?thesis by simp
          qed
      ...
      next
        ...
        have vpq: "valid_position P (spiras_sel_position P)"
          using spiras_sel_position_spec[OF wfP ge2] by simp
        have vpQ2: "valid_position P (pos @ s)" using vpq pos_eq2 by simp
        ...
        note meas = case_two_measure[OF wfP geST pos_ne vpP, of s]

        have lRpos_le: "len_formula (subterm_at P pos) \<le> len_formula P"
          by (rule subterm_at_len_le[OF vpP])
        have lXqT_le: "len_formula (fix_at (spiras_sel_position P) True P)
                       \<le> len_formula P" by (rule fix_at_len_le)
        have lXqF_le: "len_formula (fix_at (spiras_sel_position P) False P)
                       \<le> len_formula P" by (rule fix_at_len_le)
        have ltR: "len_formula (subterm_at P pos) < 4 * rebal_kk + 4"
          using lRpos_le ltP by linarith
        have ltXqT: "len_formula (fix_at (spiras_sel_position P) True P)
                     < 4 * rebal_kk + 4"
          using lXqT_le ltP by linarith
        have ltXqF: "len_formula (fix_at (spiras_sel_position P) False P)
                     < 4 * rebal_kk + 4"
          using lXqF_le ltP by linarith
        ...
        obtain lR szR depR where IH_R:
          ...
          and IH_R_l: "lR \<le> rebal_gap_bnd
                        (rebal_measure (subterm_at P pos) s)"
          and IH_R_s: "szR \<le> rebal_gap_bnd
                        (rebal_measure (subterm_at P pos) s)"
          and IH_R_d: "depR \<le> rebal_gap_bnd
                        (rebal_measure (subterm_at P pos) s)"
          using less.hyps[OF conjunct1[OF meas]
                           subterm_at_wf[OF wfP vpP]
                           conjunct2[OF vsplit] ltR] by blast
        obtain lT szT depT where IH_T:
            "provable_balanced_iff
               (spira_trans (fix_at (spiras_sel_position P) True P))
               (rebalancing (fix_at (spiras_sel_position P) True P) pos)
               lT szT depT"
          and IH_T_l: "lT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) True P) pos)"
          and IH_T_s: "szT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) True P) pos)"
          and IH_T_d: "depT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) True P) pos)"
          using less.hyps[OF conjunct1[OF conjunct2[OF meas]]
                           fix_at_wf[OF wfP] vp_Qb ltXqT] by blast
        obtain lF szF depF where IH_F:
            "provable_balanced_iff
               (spira_trans (fix_at (spiras_sel_position P) False P))
               (rebalancing (fix_at (spiras_sel_position P) False P) pos)
               lF szF depF"
          and IH_F_l: "lF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) False P) pos)"
          and IH_F_s: "szF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) False P) pos)"
          and IH_F_d: "depF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) False P) pos)"
          using less.hyps[OF conjunct2[OF conjunct2[OF meas]]
                           fix_at_wf[OF wfP] vp_Qb ltXqF] by blast
        let ?Q = "subterm_at P (spiras_sel_position P)"
        let ?R = "subterm_at P pos"
        obtain sz dep where pbi:
            "provable_balanced_iff (spira_trans P) (rebalancing P pos)
               (lT + lF + lR + case_two_glue_lines) sz dep"
          and conS: "sz \<le> szT + szF + szR + rebal_glue_coeff
              * (len_formula (spira_trans (fix_at (spiras_sel_position P) True P))
                 + len_formula (spira_trans (fix_at (spiras_sel_position P) False P))
                 + len_formula (spira_trans (fix_at pos True P))
                 + len_formula (spira_trans (fix_at pos False P))
                 ...
                 + len_formula (spira_trans ?Q)
                 + len_formula (spira_trans ?R) + 1)"
          and conD: "dep \<le> max depT (max depF (max depR (rebal_dep_coeff
              * (depth_formula (spira_trans (fix_at (spiras_sel_position P) True P))
                 + depth_formula (spira_trans (fix_at (spiras_sel_position P) False P))
                 + depth_formula (spira_trans (fix_at pos True P))
                 + depth_formula (spira_trans (fix_at pos False P))
        ...
        qed
        then obtain m1 where m1_eq: "?meas = Suc m1" by (cases ?meas) auto
        have mR_le: "rebal_measure (subterm_at P pos) s \<le> m1"
          using conjunct1[OF meas] m1_eq by simp
        have mT_le: "rebal_measure (fix_at (spiras_sel_position P) True P) pos \<le> m1"
          using conjunct1[OF conjunct2[OF meas]] m1_eq by simp
        have mF_le: "rebal_measure (fix_at (spiras_sel_position P) False P) pos \<le> m1"
          using conjunct2[OF conjunct2[OF meas]] m1_eq by simp

        have lR_b: "lR \<le> rebal_gap_bnd m1"
          using IH_R_l rebal_gap_bnd_mono[OF mR_le] by (rule order_trans)
        have lT_b: "lT \<le> rebal_gap_bnd m1"
          using IH_T_l rebal_gap_bnd_mono[OF mT_le] by (rule order_trans)
        have lF_b: "lF \<le> rebal_gap_bnd m1"
          using IH_F_l rebal_gap_bnd_mono[OF mF_le] by (rule order_trans)
        have sR_b: "szR \<le> rebal_gap_bnd m1"
          using IH_R_s rebal_gap_bnd_mono[OF mR_le] by (rule order_trans)
        have sT_b: "szT \<le> rebal_gap_bnd m1"
          using IH_T_s rebal_gap_bnd_mono[OF mT_le] by (rule order_trans)
        have sF_b: "szF \<le> rebal_gap_bnd m1"
          using IH_F_s rebal_gap_bnd_mono[OF mF_le] by (rule order_trans)
        have dR_b: "depR \<le> rebal_gap_bnd m1"
          using IH_R_d rebal_gap_bnd_mono[OF mR_le] by (rule order_trans)
        have dT_b: "depT \<le> rebal_gap_bnd m1"
          using IH_T_d rebal_gap_bnd_mono[OF mT_le] by (rule order_trans)
        have dF_b: "depF \<le> rebal_gap_bnd m1"
          using IH_F_d rebal_gap_bnd_mono[OF mF_le] by (rule order_trans)
        ...
        have wf_F: "formula_well_formed (alphabet F) (fix_at pos False P)"
          by (rule fix_at_wf[OF wfP])
        have wf_R: "formula_well_formed (alphabet F) ?R"
          by (rule subterm_at_wf[OF wfP vpP])
        have wf_sRT: "formula_well_formed (alphabet F) (fix_at s True ?R)"
          by (rule fix_at_wf[OF wf_R])
        have wf_sRF: "formula_well_formed (alphabet F) (fix_at s False ?R)"
          by (rule fix_at_wf[OF wf_R])
        have wf_Q: "formula_well_formed (alphabet F) ?Q"
          using subterm_at_wf[OF wfP] vpq by blast
        have lT_le_P: "len_formula (fix_at pos True P) \<le> len_formula P"
          by (rule fix_at_len_le)
        have lF_le_P: "len_formula (fix_at pos False P) \<le> len_formula P"
          by (rule fix_at_len_le)
        have lsRT: "len_formula (fix_at s True ?R) \<le> len_formula P"
          using fix_at_len_le lRpos_le by (rule order_trans)
        have lsRF: "len_formula (fix_at s False ?R) \<le> len_formula P"
          using fix_at_len_le lRpos_le by (rule order_trans)
        have lQ_le_P: "len_formula ?Q \<le> len_formula P"
          by (rule subterm_at_len_le[OF vpq])
        ...
        have llsum_le: "?llsum \<le> 8 * rebal_gap_Sbound"
          using leafSb[OF wf_Xq_T lXqT_le] leafSb[OF wf_Xq_F lXqF_le]
                leafSb[OF wf_T lT_le_P] leafSb[OF wf_F lF_le_P]
                leafSb[OF wf_sRT lsRT] leafSb[OF wf_sRF lsRF]
                leafSb[OF wf_Q lQ_le_P] leafSb[OF wf_R lRpos_le]
          by linarith
        let ?dlsum =
          "depth_formula (spira_trans (fix_at (spiras_sel_position P) True P))
         + depth_formula (spira_trans (fix_at (spiras_sel_position P) False P))
         ...
         + depth_formula (spira_trans ?R)"
        have dlsum_le: "?dlsum \<le> 8 * rebal_gap_Sbound"
          using leafDb[OF wf_Xq_T lXqT_le] leafDb[OF wf_Xq_F lXqF_le]
                leafDb[OF wf_T lT_le_P] leafDb[OF wf_F lF_le_P]
                leafDb[OF wf_sRT lsRT] leafDb[OF wf_sRF lsRF]
                leafDb[OF wf_Q lQ_le_P] leafDb[OF wf_R lRpos_le]
          by linarith

        have lines_le: "lT + lF + lR + case_two_glue_lines \<le> ?bnd"
        proof -
          have "lT + lF + lR + case_two_glue_lines
                \<le> 3 * rebal_gap_bnd m1 + case_two_glue_lines"
            using lT_b lF_b lR_b by linarith
          also have "\<dots> \<le> 4 * rebal_gap_bnd m1 + rebal_gap_K_step"
          proof -
            have "case_two_glue_lines \<le> rebal_gap_K_step"
              unfolding rebal_gap_K_step_def by simp
            thus ?thesis by simp
          qed
          also have "\<dots> \<le> rebal_gap_bnd (Suc m1)" by (rule rebal_gap_bnd_step)
        ...
        have sz_le: "sz \<le> ?bnd"
        proof -
          have "sz \<le> szT + szF + szR + rebal_glue_coeff * (?llsum + 1)"
            by (rule conS)
          also have "\<dots> \<le> 3 * rebal_gap_bnd m1
                        + rebal_glue_coeff * (8 * rebal_gap_Sbound + 1)"
          proof -
            have c1: "szT + szF + szR \<le> 3 * rebal_gap_bnd m1"
              using sT_b sF_b sR_b by linarith
            have c2: "?llsum + 1 \<le> 8 * rebal_gap_Sbound + 1"
              using llsum_le by linarith
            have c3: "rebal_glue_coeff * (?llsum + 1)
                      \<le> rebal_glue_coeff * (8 * rebal_gap_Sbound + 1)"
        ...
        have dep_le: "dep \<le> ?bnd"
        proof -
          have "dep \<le> max depT (max depF (max depR
                       (rebal_dep_coeff * (?dlsum + 1))))" by (rule conD)
          also have "\<dots> \<le> rebal_gap_bnd m1
                        + rebal_dep_coeff * (8 * rebal_gap_Sbound + 1)"
          proof -
            have a1: "depT \<le> rebal_gap_bnd m1" by (rule dT_b)
            have a2: "depF \<le> rebal_gap_bnd m1" by (rule dF_b)
            have a3: "depR \<le> rebal_gap_bnd m1" by (rule dR_b)
            have a4: "rebal_dep_coeff * (?dlsum + 1)
                      \<le> rebal_dep_coeff * (8 * rebal_gap_Sbound + 1)"
              using dlsum_le by simp
            from a1 a2 a3 a4 show ?thesis by linarith
      ...
      next
        ...
        have vp_Qb_pos: "\<And>b. valid_position (fix_at (spiras_sel_position P) b P) pos"
          by (rule valid_position_fix_at_disjoint[OF dpq vpP])
        have vp_Rb_q: "\<And>b. valid_position (fix_at pos b P) (spiras_sel_position P)"
        ...
        have lF_le_P: "len_formula (fix_at pos False P) \<le> len_formula P"
          by (rule fix_at_len_le)
        have ltXqT: "len_formula (fix_at (spiras_sel_position P) True P)
                     < 4 * rebal_kk + 4" using lXqT_le ltP by linarith
        have ltXqF: "len_formula (fix_at (spiras_sel_position P) False P)
                     < 4 * rebal_kk + 4" using lXqF_le ltP by linarith
        have ltT: "len_formula (fix_at pos True P) < 4 * rebal_kk + 4"
          using lT_le_P ltP by linarith
        have ltF: "len_formula (fix_at pos False P) < 4 * rebal_kk + 4"
          using lF_le_P ltP by linarith
        ...
        obtain lQT szQT depQT where IH_QT:
            "provable_balanced_iff
               ...
               (rebalancing (fix_at (spiras_sel_position P) True P) pos)
               lQT szQT depQT"
          and IH_QT_l: "lQT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) True P) pos)"
          and IH_QT_s: "szQT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) True P) pos)"
          and IH_QT_d: "depQT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) True P) pos)"
          using less.hyps[OF conjunct1[OF meas]
                           fix_at_wf[OF wfP] vp_Qb_pos ltXqT] by blast
        obtain lQF szQF depQF where IH_QF:
            "provable_balanced_iff
               (spira_trans (fix_at (spiras_sel_position P) False P))
               (rebalancing (fix_at (spiras_sel_position P) False P) pos)
               lQF szQF depQF"
          and IH_QF_l: "lQF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) False P) pos)"
          and IH_QF_s: "szQF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) False P) pos)"
          and IH_QF_d: "depQF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at (spiras_sel_position P) False P) pos)"
          using less.hyps[OF conjunct1[OF conjunct2[OF meas]]
                           fix_at_wf[OF wfP] vp_Qb_pos ltXqF] by blast
        obtain lRT szRT depRT where IH_RT:
            "provable_balanced_iff (spira_trans (fix_at pos True P))
               (rebalancing (fix_at pos True P) (spiras_sel_position P))
               lRT szRT depRT"
          and IH_RT_l: "lRT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos True P) (spiras_sel_position P))"
          and IH_RT_s: "szRT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos True P) (spiras_sel_position P))"
          and IH_RT_d: "depRT \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos True P) (spiras_sel_position P))"
          using less.hyps[OF conjunct1[OF conjunct2[OF conjunct2[OF meas]]]
                           fix_at_wf[OF wfP] vp_Rb_q ltT] by blast
        obtain lRF szRF depRF where IH_RF:
            "provable_balanced_iff (spira_trans (fix_at pos False P))
               (rebalancing (fix_at pos False P) (spiras_sel_position P))
               lRF szRF depRF"
          and IH_RF_l: "lRF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos False P) (spiras_sel_position P))"
          and IH_RF_s: "szRF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos False P) (spiras_sel_position P))"
          and IH_RF_d: "depRF \<le> rebal_gap_bnd
                        (rebal_measure (fix_at pos False P) (spiras_sel_position P))"
          using less.hyps[OF conjunct2[OF conjunct2[OF conjunct2[OF meas]]]
                           fix_at_wf[OF wfP] vp_Rb_q ltF] by blast

        let ?Q = "subterm_at P (spiras_sel_position P)"
        let ?R = "subterm_at P pos"
        let ?gtt = "fix_at (spiras_sel_position P) True (fix_at pos True P)"
        let ?gtf = "fix_at (spiras_sel_position P) True (fix_at pos False P)"
        let ?gft = "fix_at (spiras_sel_position P) False (fix_at pos True P)"
        let ?gff = "fix_at (spiras_sel_position P) False (fix_at pos False P)"

        obtain sz dep where pbi:
            "provable_balanced_iff (spira_trans P) (rebalancing P pos)
               (lQT + lQF + lRT + lRF + case_three_glue_lines) sz dep"
          and conS: "sz \<le> szQT + szQF + szRT + szRF + rebal_glue_coeff3
              * (len_formula (spira_trans (fix_at (spiras_sel_position P) True P))
                 + len_formula (spira_trans (fix_at (spiras_sel_position P) False P))
                 + len_formula (spira_trans (fix_at pos True P))
                 + len_formula (spira_trans (fix_at pos False P))
                 + len_formula (spira_trans ?gtt)
                 + len_formula (spira_trans ?gtf)
                 + len_formula (spira_trans ?gft)
                 + len_formula (spira_trans ?gff)
                 + len_formula (spira_trans ?Q)
                 + len_formula (spira_trans ?R) + 1)"
          and conD: "dep \<le> max depQT (max depQF (max depRT (max depRF
              (rebal_dep_coeff3
              * (depth_formula (spira_trans (fix_at (spiras_sel_position P) True P))
                 + depth_formula (spira_trans (fix_at (spiras_sel_position P) False P))
                 + depth_formula (spira_trans (fix_at pos True P))
                 + depth_formula (spira_trans (fix_at pos False P))
                 + depth_formula (spira_trans ?gtt)
                 + depth_formula (spira_trans ?gtf)
                 + depth_formula (spira_trans ?gft)
                 + depth_formula (spira_trans ?gff)
                 + depth_formula (spira_trans ?Q)
        ...
        qed
        then obtain m1 where m1_eq: "?meas = Suc m1" by (cases ?meas) auto
        have mQT_le: "rebal_measure (fix_at (spiras_sel_position P) True P) pos
                      \<le> m1"
          using conjunct1[OF meas] m1_eq by simp
        have mQF_le: "rebal_measure (fix_at (spiras_sel_position P) False P) pos
                      \<le> m1"
          using conjunct1[OF conjunct2[OF meas]] m1_eq by simp
        have mRT_le: "rebal_measure (fix_at pos True P) (spiras_sel_position P)
                      \<le> m1"
          using conjunct1[OF conjunct2[OF conjunct2[OF meas]]] m1_eq by simp
        have mRF_le: "rebal_measure (fix_at pos False P) (spiras_sel_position P)
                      \<le> m1"
          using conjunct2[OF conjunct2[OF conjunct2[OF meas]]] m1_eq by simp

        have lQT_b: "lQT \<le> rebal_gap_bnd m1"
          using IH_QT_l rebal_gap_bnd_mono[OF mQT_le] by (rule order_trans)
        have lQF_b: "lQF \<le> rebal_gap_bnd m1"
          using IH_QF_l rebal_gap_bnd_mono[OF mQF_le] by (rule order_trans)
        have lRT_b: "lRT \<le> rebal_gap_bnd m1"
          using IH_RT_l rebal_gap_bnd_mono[OF mRT_le] by (rule order_trans)
        have lRF_b: "lRF \<le> rebal_gap_bnd m1"
          using IH_RF_l rebal_gap_bnd_mono[OF mRF_le] by (rule order_trans)
        have sQT_b: "szQT \<le> rebal_gap_bnd m1"
          using IH_QT_s rebal_gap_bnd_mono[OF mQT_le] by (rule order_trans)
        have sQF_b: "szQF \<le> rebal_gap_bnd m1"
          using IH_QF_s rebal_gap_bnd_mono[OF mQF_le] by (rule order_trans)
        have sRT_b: "szRT \<le> rebal_gap_bnd m1"
          using IH_RT_s rebal_gap_bnd_mono[OF mRT_le] by (rule order_trans)
        have sRF_b: "szRF \<le> rebal_gap_bnd m1"
          using IH_RF_s rebal_gap_bnd_mono[OF mRF_le] by (rule order_trans)
        have dQT_b: "depQT \<le> rebal_gap_bnd m1"
          using IH_QT_d rebal_gap_bnd_mono[OF mQT_le] by (rule order_trans)
        have dQF_b: "depQF \<le> rebal_gap_bnd m1"
          using IH_QF_d rebal_gap_bnd_mono[OF mQF_le] by (rule order_trans)
        have dRT_b: "depRT \<le> rebal_gap_bnd m1"
          using IH_RT_d rebal_gap_bnd_mono[OF mRT_le] by (rule order_trans)
        have dRF_b: "depRF \<le> rebal_gap_bnd m1"
          using IH_RF_d rebal_gap_bnd_mono[OF mRF_le] by (rule order_trans)
        ...
        have wf_F: "formula_well_formed (alphabet F) (fix_at pos False P)"
          by (rule fix_at_wf[OF wfP])
        have wf_gtt: "formula_well_formed (alphabet F) ?gtt"
          by (rule fix_at_wf[OF wf_T])
        have wf_gtf: "formula_well_formed (alphabet F) ?gtf"
          by (rule fix_at_wf[OF wf_F])
        have wf_gft: "formula_well_formed (alphabet F) ?gft"
          by (rule fix_at_wf[OF wf_T])
        have wf_gff: "formula_well_formed (alphabet F) ?gff"
          by (rule fix_at_wf[OF wf_F])
        have wf_Q: "formula_well_formed (alphabet F) ?Q"
          using subterm_at_wf[OF wfP] vpq by blast
        have wf_R: "formula_well_formed (alphabet F) ?R"
          by (rule subterm_at_wf[OF wfP vpP])
        have lgtt: "len_formula ?gtt \<le> len_formula P"
          using fix_at_len_le lT_le_P by (rule order_trans)
        have lgtf: "len_formula ?gtf \<le> len_formula P"
          using fix_at_len_le lF_le_P by (rule order_trans)
        have lgft: "len_formula ?gft \<le> len_formula P"
          using fix_at_len_le lT_le_P by (rule order_trans)
        have lgff: "len_formula ?gff \<le> len_formula P"
          using fix_at_len_le lF_le_P by (rule order_trans)
        have lQ_le_P: "len_formula ?Q \<le> len_formula P"
          by (rule subterm_at_len_le[OF vpq])
        have lR_le_P: "len_formula ?R \<le> len_formula P"
          by (rule subterm_at_len_le[OF vpP])
        ...
        let ?llsum =
         ...
         + len_formula (spira_trans ?Q)
         + len_formula (spira_trans ?R)"
        have llsum_le: "?llsum \<le> 10 * rebal_gap_Sbound"
          using leafSb[OF wf_Xq_T lXqT_le] leafSb[OF wf_Xq_F lXqF_le]
                leafSb[OF wf_T lT_le_P] leafSb[OF wf_F lF_le_P]
                leafSb[OF wf_gtt lgtt] leafSb[OF wf_gtf lgtf]
                leafSb[OF wf_gft lgft] leafSb[OF wf_gff lgff]
                leafSb[OF wf_Q lQ_le_P] leafSb[OF wf_R lR_le_P]
          by linarith
        let ?dlsum =
          "depth_formula (spira_trans (fix_at (spiras_sel_position P) True P))
         + depth_formula (spira_trans (fix_at (spiras_sel_position P) False P))
         ...
         + depth_formula (spira_trans ?Q)
         + depth_formula (spira_trans ?R)"
        have dlsum_le: "?dlsum \<le> 10 * rebal_gap_Sbound"
          using leafDb[OF wf_Xq_T lXqT_le] leafDb[OF wf_Xq_F lXqF_le]
                leafDb[OF wf_T lT_le_P] leafDb[OF wf_F lF_le_P]
                leafDb[OF wf_gtt lgtt] leafDb[OF wf_gtf lgtf]
                leafDb[OF wf_gft lgft] leafDb[OF wf_gff lgff]
                leafDb[OF wf_Q lQ_le_P] leafDb[OF wf_R lR_le_P]
          by linarith

        have lines_le: "lQT + lQF + lRT + lRF + case_three_glue_lines \<le> ?bnd"
        proof -
          have "lQT + lQF + lRT + lRF + case_three_glue_lines
                \<le> 4 * rebal_gap_bnd m1 + case_three_glue_lines"
            using lQT_b lQF_b lRT_b lRF_b by linarith
          also have "\<dots> \<le> 4 * rebal_gap_bnd m1 + rebal_gap_K_step"
          proof -
            have "case_three_glue_lines \<le> rebal_gap_K_step"
              unfolding rebal_gap_K_step_def by simp
            thus ?thesis by simp
          qed
          also have "\<dots> \<le> rebal_gap_bnd (Suc m1)" by (rule rebal_gap_bnd_step)
        ...
        have sz_le: "sz \<le> ?bnd"
        proof -
          have "sz \<le> szQT + szQF + szRT + szRF + rebal_glue_coeff3 * (?llsum + 1)"
            by (rule conS)
          also have "\<dots> \<le> 4 * rebal_gap_bnd m1
                        + rebal_glue_coeff3 * (10 * rebal_gap_Sbound + 1)"
          proof -
            have c1: "szQT + szQF + szRT + szRF \<le> 4 * rebal_gap_bnd m1"
              using sQT_b sQF_b sRT_b sRF_b by linarith
            have c2: "?llsum + 1 \<le> 10 * rebal_gap_Sbound + 1"
              using llsum_le by linarith
            have c3: "rebal_glue_coeff3 * (?llsum + 1)
                      \<le> rebal_glue_coeff3 * (10 * rebal_gap_Sbound + 1)"
              using c2 by simp
            from c1 c3 show ?thesis by linarith
          qed
          also have "\<dots> \<le> 4 * rebal_gap_bnd m1 + rebal_gap_K_step"
          proof -
            have "rebal_glue_coeff3 * (10 * rebal_gap_Sbound + 1)
                  \<le> rebal_gap_K_step"
              unfolding rebal_gap_K_step_def by simp
            thus ?thesis by simp
          qed
        ...
        have dep_le: "dep \<le> ?bnd"
        proof -
          have "dep \<le> max depQT (max depQF (max depRT (max depRF
                       (rebal_dep_coeff3 * (?dlsum + 1)))))" by (rule conD)
          also have "\<dots> \<le> rebal_gap_bnd m1
                        + rebal_dep_coeff3 * (10 * rebal_gap_Sbound + 1)"
          proof -
            have a1: "depQT \<le> rebal_gap_bnd m1" by (rule dQT_b)
            have a2: "depQF \<le> rebal_gap_bnd m1" by (rule dQF_b)
            have a3: "depRT \<le> rebal_gap_bnd m1" by (rule dRT_b)
            have a4: "depRF \<le> rebal_gap_bnd m1" by (rule dRF_b)
            have a5: "rebal_dep_coeff3 * (?dlsum + 1)
                      \<le> rebal_dep_coeff3 * (10 * rebal_gap_Sbound + 1)"
              using dlsum_le by simp
            from a1 a2 a3 a4 a5 show ?thesis by linarith
          qed
          also have "\<dots> \<le> 4 * rebal_gap_bnd m1 + rebal_gap_K_step"
          proof -
            have "rebal_dep_coeff3 * (10 * rebal_gap_Sbound + 1)
                  \<le> rebal_gap_K_step"
              unfolding rebal_gap_K_step_def by simp
            thus ?thesis by simp
          qed
...
definition rebal_gapc :: nat where
  "rebal_gapc = (SOME m. \<forall> P pos.
       formula_well_formed (alphabet F) P \<and> valid_position P pos
...
\<comment> \<open>rebal_measure P pos is bounded by a constant when |P| < 4*rebal_kk+4.
    Specifically: rebal_measure P pos = |P|(|P|+1) + (|P|-|R|) \<le> B(B+1) + B
    where B = 4*rebal_kk+3.  Then rebal_gap_bnd at that constant gives a
    uniform bound, since rebal_gap_bnd is monotone.\<close>
lemma rebal_gapc_ex:
  "\<exists> m. \<forall> P pos.
       formula_well_formed (alphabet F) P \<and> valid_position P pos
       \<and> spira_threshold \<le> len_formula P
...
lemma rebal_gapc_spec:
  ...
  shows "\<exists> lines sz dep.
           provable_balanced_iff (spira_trans P) (rebalancing P pos)
             lines sz dep
         \<and> lines \<le> rebal_gapc \<and> sz \<le> rebal_gapc \<and> dep \<le> rebal_gapc"
  using someI_ex[OF rebal_gapc_ex] assms unfolding rebal_gapc_def by blast
...
definition rebal_base_K :: nat where
  "rebal_base_K = case_one_glue_lines + case_two_glue_lines
                  ...
                  + refl_step_len + pos_empty_step_len + rebal_shc + rebal_gapc
...
proof -
  ...
  define c :: real where
    c_def: "c = real (refl_step_depth + pos_empty_step_depth + 1)
                + max tc 1 + real rebal_shc + real rebal_gapc
  ...
  proof -
    ...
    proof -
      ...
      proof -
        have nn: "(0::real) \<le> (real rebal_shc + real rebal_gapc
              + real rebal_dep_coeff * (8 * max tc 1 + 1)
    ...
    next
      ...
      proof (cases "len_formula P < 4 * rebal_kk + 4")
        case True
        \<comment> \<open>Gap region: rebal_gapc_spec.\<close>
        obtain lines sz dep where
            sc: "provable_balanced_iff (spira_trans P) (rebalancing P pos)
                   lines sz dep"
          and scl: "lines \<le> rebal_gapc"
          and scs: "sz \<le> rebal_gapc"
          and scd: "dep \<le> rebal_gapc"
          using rebal_gapc_spec[OF wfP vpP geST True] by blast
        have gapc_L: "rebal_gapc \<le> rebal_L (len_formula P) (rebal_m P pos)"
          by (rule rebal_L_dom_const[OF _ lP1]) (simp add: rebal_base_K_def)
        have lb: "lines \<le> rebal_L (len_formula P) (rebal_m P pos)"
          using scl gapc_L by linarith
        have sb: "sz \<le> rebal_L (len_formula P) (rebal_m P pos)"
          using scs gapc_L by linarith
        have db: "real dep \<le> c * log 2 (real (len_formula P) + 1)"
        proof -
          have "real dep \<le> real rebal_gapc" using scd by simp
          also have "\<dots> \<le> c" unfolding c_def by simp
          finally have "real dep \<le> c" .
          thus ?thesis by (rule const_to_c)
        qed
