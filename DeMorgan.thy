...
proof -
  ...
  proof (intro ballI allI impI)
    ...
    have len_sub_bound: "len_sub (var_set_rule rule) sub \<le> ?c * len_proof pr"
      using de_morgan_frege.len_sub_bound_by_proof[OF dm2 r_in dwith rsub, of ?c] valid_pr by simp
...
lemma step_F_proves:
  assumes "r \<in> rules F"
  shows "valid_proof F' (step_F r s) \<and>
...
lemma sim_left_step_progress:
  ...
  shows "assumptions (sim_left_step pr k acc) = {} \<and>
         valid_proof F' (sim_left_step pr k acc) \<and>
         thesis (sim_left_step pr k acc) = steps pr ! k \<and>
         set (take (Suc k) (steps pr)) \<subseteq> set (steps (sim_left_step pr k acc))"
proof -
  let ?step = "steps pr ! k"
  ...
  qed
  have eq: "sim_left_step pr k acc = combine_proofs acc (step_F ?r ?s)"
  proof (cases "choose_rule_sub F k pr")
    case (Pair r s)
    then show ?thesis
      using step_not_assm k_lt by (simp add: sim_left_step_def Let_def)
  qed
  have valid_next: "valid_proof F' (sim_left_step pr k acc)"
  proof (cases "k = 0")
    case True
    ...
    have comb_eq: "combine_proofs acc (step_F ?r ?s) = step_F ?r ?s"
      using acc_assm acc_steps0 step_props by (cases acc) simp
    have "sim_left_step pr k acc = combine_proofs acc (step_F ?r ?s)"
      using eq .
    also have "\<dots> = step_F ?r ?s"
      using comb_eq .
    finally have "sim_left_step pr k acc = step_F ?r ?s" .
    then show ?thesis
  ...
  next
    ...
    show ?thesis
      ...
      by blast
  qed
  have assm_next: "assumptions (sim_left_step pr k acc) = {}"
    using eq acc_assm step_props prems_seen by simp
  have steps_next: "set (take (Suc k) (steps pr)) \<subseteq> set (steps (sim_left_step pr k acc))"
  proof
    fix f
    ...
    then show "f \<in> set (steps (sim_left_step pr k acc))"
    proof cases
      case 1
    ...
    next
      ...
      moreover have "thesis (step_F ?r ?s) = concl (sub_rule ?s ?r)"
        using step_props by blast
      moreover have "valid_proof F' (step_F ?r ?s)"
        using step_props by blast
      then have "thesis (step_F ?r ?s) \<in> set (steps (step_F ?r ?s))"
        unfolding valid_proof_def by (metis last_in_set)
      ultimately show ?thesis
        using 2 fi eq by auto
    qed
  qed
  have thesis_next: "thesis (sim_left_step pr k acc) = steps pr ! k"
  proof -
    have "concl (sub_rule ?s ?r) = steps pr ! k"
      using dchoose unfolding derived_with_def by simp
  ...
  proof (intro allI impI)
    ...
    next
      ...
      have acc0: "k = 0 \<longrightarrow> steps (?acc k) = []"
        by (cases k) simp_all
      have step_prog:
        "assumptions (sim_left_step pr k (?acc k)) = {} \<and>
         valid_proof F' (sim_left_step pr k (?acc k)) \<and>
         thesis (sim_left_step pr k (?acc k)) = steps pr ! k \<and>
         set (take (Suc k) (steps pr)) \<subseteq> set (steps (sim_left_step pr k (?acc k)))"
        using sim_left_step_progress[of pr k "?acc k"] assms k_lt acc0 IH by blast
      show ?case
...
lemma step_F_bound:
  shows "\<exists> g :: nat poly. \<forall> rule \<in> rules F. \<forall>  pr i sub. 
            derived_with i pr rule sub \<and> rule_restricted_sub rule sub \<and> valid_proof F pr \<longrightarrow> 
            len_proof (step_F rule sub) \<le> poly g (len_proof pr)"
proof -
  let ?c = "Max ((\<lambda>r. card (var_set_rule r)) ` rules F) + 1"
  let ?S = "(\<lambda>r. ?c * len_proof (rule_proof_fun_F r)) ` rules F"
  let ?g = "[:0, Max ?S:]"
  have fin_rules: "finite (rules F)"
  ...
  proof (rule exI[of _ ?g], intro ballI allI impI)
    ...
    have restricted_sub: "\<forall>v. v \<notin> var_set_rule rule \<longrightarrow> sub v = Atom v"
      using rsub unfolding rule_restricted_sub_def by simp
    have step_bound:
      "len_proof (step_F rule sub)
       \<le> len_proof (rule_proof_fun_F rule) * len_sub (var_set_rule rule) sub"
      unfolding step_F.simps
      using sub_proof_bound[of "var_set_rule rule" sub "rule_proof_fun_F rule"] var_set_rule_finite[of rule] restricted_sub
      by simp
    have step_bound':
      "len_proof (step_F rule sub)
       \<le> (?c * len_proof (rule_proof_fun_F rule)) * len_proof pr"
    proof -
      have coeff_nonneg: "0 \<le> len_proof (rule_proof_fun_F rule)"
        by simp
      have mult_bound:
        "len_proof (rule_proof_fun_F rule) * len_sub (var_set_rule rule) sub
         \<le> len_proof (rule_proof_fun_F rule) * (?c * len_proof pr)"
        using len_sub_bound coeff_nonneg by (rule mult_left_mono)
      have "len_proof (step_F rule sub) \<le> len_proof (rule_proof_fun_F rule) * (?c * len_proof pr)"
        using step_bound mult_bound by linarith
      then show ?thesis
        by (simp add: algebra_simps)
    qed
    have coeff_in: "?c * len_proof (rule_proof_fun_F rule) \<in> ?S"
      using r_in by simp
    have coeff_le: "?c * len_proof (rule_proof_fun_F rule) \<le> Max ?S"
      using Max_ge[OF fin_S coeff_in] by simp
    have mono: "(?c * len_proof (rule_proof_fun_F rule)) * len_proof pr \<le> Max ?S * len_proof pr"
      using coeff_le by (simp add: mult_right_mono)
    have "len_proof (step_F rule sub) \<le> Max ?S * len_proof pr"
      using step_bound' mono by linarith
    then show "len_proof (step_F rule sub) \<le> poly ?g (len_proof pr)"
      by (simp add: algebra_simps)
  qed
qed

lemma sim_left_step_bound:
  shows "\<exists> bound. \<forall>pr i acc. i \<ge> 0 \<and> i < length (steps pr) \<and> valid_proof F pr \<and> assumptions pr = {} \<longrightarrow>
            len_proof (sim_left_step pr i acc) \<le> poly bound (len_proof pr) + len_proof acc"
proof -
  obtain g :: "nat poly" where g_prop:
    "\<forall>rule\<in>rules F. \<forall>pr i sub.
      derived_with i pr rule sub \<and> rule_restricted_sub rule sub \<and> valid_proof F pr \<longrightarrow>
      len_proof (step_F rule sub) \<le> poly g (len_proof pr)"
    using step_F_bound by blast
  let ?bound = "[:0, 1:] + g"
  show ?thesis
  proof (rule exI[of _ ?bound], intro allI impI)
    fix pr i acc
    ...
    have rsub: "rule_restricted_sub ?rule ?sub"
      using choose_props by simp
    have step_b: "len_proof (step_F ?rule ?sub) \<le> poly g (len_proof pr)"
      using g_prop r_in dwith rsub pr_valid by blast
    have step_eq: "sim_left_step pr i acc = combine_proofs acc (step_F ?rule ?sub)"
    proof (cases "choose_rule_sub F i pr")
      case (Pair r s)
      then show ?thesis
        using step_notin i_lt by (simp add: sim_left_step_def Let_def)
    qed
    have "len_proof (sim_left_step pr i acc) = len_proof acc + len_proof (step_F ?rule ?sub)"
      using step_eq by simp
    also have "\<dots> \<le> len_proof acc + poly g (len_proof pr)"
      using step_b by simp
    also have "\<dots> \<le> len_proof acc + poly ?bound (len_proof pr)"
      by simp
    finally show "len_proof (sim_left_step pr i acc) \<le> poly ?bound (len_proof pr) + len_proof acc"
      by (simp add: add.assoc add.commute add.left_commute)
  qed
qed

lemma sim_left_bound:
  assumes "valid_proof F pr \<and> assumptions pr = {}"
  shows "\<exists> bound. len_proof (sim_left pr (thesis pr)) \<le> poly bound (len_proof pr)"
proof -
  obtain g :: "nat poly" where g_prop:
    "\<forall>pr i acc. i \<ge> 0 \<and> i < length (steps pr) \<and> valid_proof F pr \<and> assumptions pr = {} \<longrightarrow>
      len_proof (sim_left_step pr i acc) \<le> poly g (len_proof pr) + len_proof acc"
    using sim_left_step_bound by blast
  let ?acc = "\<lambda>k. fold (sim_left_step pr) [0..<k] \<lparr>assumptions = assumptions pr, thesis = thesis pr, steps = []\<rparr>"
  have pr_valid: "valid_proof F pr"
    using assms by simp
  have pr_assm: "assumptions pr = {}"
    using assms by simp
  ...
  proof (intro allI impI)
    ...
    next
      ...
      have ih: "len_proof (?acc k) \<le> k * poly g (len_proof pr)"
        using Suc.IH Suc.prems by simp
      have step_b:
        "len_proof (sim_left_step pr k (?acc k)) \<le> poly g (len_proof pr) + len_proof (?acc k)"
        using g_prop k_lt pr_valid pr_assm by simp
      have "?acc (Suc k) = sim_left_step pr k (?acc k)"
        by simp
      then have "len_proof (?acc (Suc k)) \<le> poly g (len_proof pr) + len_proof (?acc k)"
        using step_b by simp
      also have "\<dots> \<le> poly g (len_proof pr) + k * poly g (len_proof pr)"
  ...
  let ?bound = "[:0, 1:] * g"
  have final_fold:
    "len_proof (sim_left pr (thesis pr)) \<le> length (steps pr) * poly g (len_proof pr)"
    unfolding sim_left_def using fold_bound by simp
  also have "\<dots> \<le> len_proof pr * poly g (len_proof pr)"
    using steps_bound by simp
  also have "\<dots> = poly ?bound (len_proof pr)"
    by simp
...
proof -
  obtain g :: "nat poly" where g_prop:
    "\<forall>pr i acc. i \<ge> 0 \<and> i < length (steps pr) \<and> valid_proof F pr \<and> assumptions pr = {} \<longrightarrow>
      len_proof (sim_left_step pr i acc) \<le> poly g (len_proof pr) + len_proof acc"
    using sim_left_step_bound by blast
  let ?f = "\<lambda>w \<tau>. sim_left w (thesis w)"
  let ?g = "\<lambda>\<tau>. \<tau>"
  let ?p = "[:0, 1:]"
  let ?q = "[:0, 1:] * g"
  have sim_case:
  ...
  proof
    ...
    have pr_props:
      "valid_proof F' (?f w \<tau>) \<and> thesis (?f w \<tau>) = thesis w \<and> assumptions (?f w \<tau>) = {}"
      using sim_left_proves[of w "sim_left w (thesis w)"] w_valid w_assm by simp
    let ?acc = "\<lambda>k. fold (sim_left_step w) [0..<k]
      \<lparr>assumptions = assumptions w, thesis = thesis w, steps = []\<rparr>"
    have steps_bound: "length (steps w) \<le> len_proof w"
    proof -
      have ones_le_list:
    ...
    proof (intro allI impI)
      ...
      next
        ...
        have ih: "len_proof (?acc k) \<le> k * poly g (len_proof w)"
          using Suc.IH Suc.prems by simp
        have step_b:
          "len_proof (sim_left_step w k (?acc k)) \<le> poly g (len_proof w) + len_proof (?acc k)"
          using g_prop k_lt w_valid w_assm by simp
        have "?acc (Suc k) = sim_left_step w k (?acc k)"
          by simp
        then have "len_proof (?acc (Suc k)) \<le> poly g (len_proof w) + len_proof (?acc k)"
          using step_b by simp
        also have "\<dots> \<le> poly g (len_proof w) + k * poly g (len_proof w)"
    ...
    have global_bound:
      "len_proof (?f w \<tau>) \<le> poly ?q (len_proof w)"
    proof -
      have "len_proof (sim_left w (thesis w)) \<le> length (steps w) * poly g (len_proof w)"
        unfolding sim_left_def using fold_bound by simp
      also have "\<dots> \<le> len_proof w * poly g (len_proof w)"
        using steps_bound by simp
      also have "\<dots> = poly ?q (len_proof w)"
