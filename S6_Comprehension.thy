      ...
      proof (rule impI)
        ...
        proof (cases "len_formula ?N1 < spira_threshold")
          ...
          \<comment> \<open>below threshold both sides are literally @{term ?N1}, so reflexivity suffices —
              this is where keeping the arity pays off: an arity-reducing form would have
              needed a separate constant-elimination identity here\<close>
    ...
    next
      ...
      have rebeq: "rebalancing (Conn conn ps) [0]
          = balance (spira_trans (Conn conn (true_const # rest)))
                    ...
                    (spira_trans Q1)"
        unfolding rebalancing_def ps_eq by simp
